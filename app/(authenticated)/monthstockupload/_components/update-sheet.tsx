...
export function UpdateRecordSheet({
  ...
  function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const isXls = selectedFile.name.endsWith(".xls");

    if (!isCsv && !isXlsx && !isXls) {
      toast.error("Please upload a CSV or XLSX or XLS file.");
      return;
    }
    ...
    if (isXlsx || isXls) {
      ...
      reader.onload = (event) => {
        ...
        const workbook = XLSX.read(data, { type: "array" }); // use 'array' here, not 'buffer'
        ...
        const jsonData = XLSX.utils.sheet_to_json<Record<string, string>>(
          worksheet,
          {
            defval: "", // optional: fill empty cells with blank
          }
        );
        ...
        setFileEditedData(JSON.parse(JSON.stringify(jsonData)));

        toast.success("XLSX converted to CSV successfully!");
      };
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-5xl">
        <DialogHeader>
          <DialogTitle>
            {isNew ? "Create Month Stock Upload" : "Update Month Stock Upload"}
          </DialogTitle>
          <DialogDescription>
            {isNew
              ? "Enter details to create a new Month Stock Upload."
              : "Update the Month Stock Upload details and save the changes."}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
