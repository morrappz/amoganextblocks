...
interface RecordsTableFloatingBarProps {
  table: Table<Contact>;
  onDelete: (ids: number[]) => void;
  onUpdateStatus: (ids: number[], status: Contact["status"]) => void;
}

export function RecordsTableFloatingBar({
  table,
  onDelete,
  onUpdateStatus,
}: RecordsTableFloatingBarProps) {
            ...
            <div className="flex items-center gap-1.5">
              <Select
                onValueChange={(value: Contact["status"]) =>
                  onUpdateStatus(
                    rows.map((row) => row.original.user_catalog_id),
                    value
                  )
                }
              >
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  onDelete(rows.map((row) => row.original.user_catalog_id))
                }
              >
