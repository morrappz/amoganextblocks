...
import type { GetMyDocsSchema } from "./validations";
import { filterSupabase } from "@/lib/filter-columns";
import { myDocStatuses } from "../type";

function applyFilters(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  query: any,
  input: GetMyDocsSchema,
  ...
  if (advancedTable) {
    ...
    if (input.doc_name) {
      query = query.ilike("doc_name", `%${input.doc_name}%`);
    }
    if (input.from) {
...
export async function getRecords(input: GetMyDocsSchema) {
  try {
    const { page, perPage, sort, flags } = input;
    const offset = (page - 1) * perPage;
    const advancedTable = flags.includes("advancedTable");

    let query = postgrest.from("mydocs").select("*");
