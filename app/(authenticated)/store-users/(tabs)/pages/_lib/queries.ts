...
import type { GetPageListsSchema } from "./validations";
...
function applyFilters(
  ...
  if (advancedTable) {
    ...
    if (input.query) {
      query = query.or(
        `page_name.ilike.%${input.query}%,page_link.ilike.%${input.query}%`
      );
    }
    if (input.from) {
...
export async function getRecords(input: GetPageListsSchema) {
  try {
    const { page, perPage, sort, flags } = input;
    const offset = (page - 1) * perPage;
    const advancedTable = flags.includes("advancedTable");

    let query = postgrest.from("page_list").select("*");
