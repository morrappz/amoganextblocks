...
import type { getStoreProducts } from "../_lib/queries";
...
interface UsersTableProps {
  promises: Promise<[Awaited<ReturnType<typeof getStoreProducts>>]>;
}
...
export function UsersTable({ promises }: UsersTableProps) {
  ...
  const [rowAction, setRowAction] =
    React.useState<DataTableRowAction<ProductType> | null>(null);
  ...
  const filterFields: DataTableFilterField<ProductType>[] = [
    {
      ...
      label: "Status",
      multiple: false,
      options: ["all", ...ProductStatuses].map((status) => ({
            ...
            endContent={
              <>
                ...
                <Button
                  size="sm"
                  onClick={() =>
                    setRowAction({ row: {} as Row<ProductType>, type: "new" })
                  }
                >
