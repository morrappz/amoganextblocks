...
import type { CreateFinalBomSchema, UpdateFinalBomSchema } from "./validations";
import { postgrest } from "@/lib/postgrest";
import { getErrorMessage } from "@/lib/handle-error";
import { auth } from "@/auth";

export async function createRecord(input: CreateFinalBomSchema) {
  unstable_noStore();
  try {
    const { ...rest } = input;
...
export async function updateRecord(
  ...
  try {
    ...
    if (data?.status === input.status) revalidateTag("final_bom-status-counts");
...
export async function deleteRecord(input: { id: number }) {
  ...
  try {
    ...
    if (uploadData?.data_upload_id) {
      const { error: dataError } = await postgrest
        .asAdmin()
        .from("final_bom_item")
        .delete()
        .eq("data_upload_id", uploadData.data_upload_id);
...
export async function deleteRecords(input: { ids: number[] }) {
  ...
  try {
    ...
    const dataUploadIds = uploadData
      ?.map((item) => item.data_upload_id)
      .filter((id): id is number => id != null);

    if (dataUploadIds && dataUploadIds.length > 0) {
      const { error: dataError } = await postgrest
        .asAdmin()
        .from("final_bom_item")
        .delete()
        .in("final_bom_id", dataUploadIds);
...
export async function saveFinalBom(input: {
  name: string;
  description?: string;
