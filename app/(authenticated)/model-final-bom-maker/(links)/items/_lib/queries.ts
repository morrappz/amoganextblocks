...
import type { GetFinalBomItemSchema } from "./validations";
import { filterSupabase } from "@/lib/filter-columns";
import { finalBomItemStatuses } from "../type";

function applyFilters(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  query: any,
  input: GetFinalBomItemSchema,
  ...
  if (advancedTable) {
    ...
    if (input.query) {
      query = query.or(
        `bom_type.ilike.%${input.query}%,bom_name.ilike.%${input.query}%,bom_code.ilike.%${input.query}%,description.ilike.%${input.query}%`
      );
    }
    if (input.from) {
...
export async function getRecords(input: GetFinalBomItemSchema) {
  ...
  try {
    ...
    let query = postgrest.from("final_bom_item")
    .select("*")
    .eq("final_bom_id", input.searchParams.final_bom_id);
    ...
    let countQuery = postgrest
      .from("final_bom_item")
      .select("*", { count: "exact", head: true }).eq("final_bom_id", input.searchParams.final_bom_id);
...
export async function getDocumentCountByField(
  field: string,
  fieldValues?: string[],
  searchParams?: { final_bom_id?: string }
) {
  try {
    const values = fieldValues
      ? fieldValues
      : field === "status"
      ? finalBomItemStatuses
      : [];
    ...
    const counts = await Promise.all(
      values.map(async (v) => {
        const { count } = await postgrest
          .from("final_bom_item")
          .select("*", { count: "exact" })
          .eq("final_bom_id", searchParams?.final_bom_id)
          .eq(field, v);
