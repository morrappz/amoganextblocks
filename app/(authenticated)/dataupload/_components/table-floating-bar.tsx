...
export function RecordsTableFloatingBar({ table }: RecordsTableFloatingBarProps) {
        ...
        <div className="w-full overflow-x-auto">
          <div className="mx-auto flex w-fit items-center gap-2 rounded-md border bg-background p-2 text-foreground shadow">
            ...
            <div className="flex items-center gap-1.5">
              <Select
                onValueChange={(value: DataUpload["status"]) => {
                  setAction("update-status");

                  startTransition(async () => {
                    const { error } = await updateRecords({
                      ids: rows.map((row) => row.original.data_upload_uuid),
                      status: value,
                    });
