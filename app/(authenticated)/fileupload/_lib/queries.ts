...
import type { GetFileUploadsSchema } from "./validations";
import { filterSupabase } from "@/lib/filter-columns";
import { FileUploadStatuses } from "../type";

function applyFilters(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  query: any,
  input: GetFileUploadsSchema,
  ...
  if (advancedTable) {
    ...
    if (input.file_name) {
      query = query.ilike("file_name", `%${input.file_name}%`);
    }
    if (input.from) {
...
export async function getRecords(input: GetFileUploadsSchema) {
  try {
    const { page, perPage, sort, flags } = input;
    const offset = (page - 1) * perPage;
    const advancedTable = flags.includes("advancedTable");

    let query = postgrest.from("file_upload").select("*");
