...
import { FormFieldList } from "../AgentBuilder/FormFieldList";
...
import { FormSettingsModal } from "./agent-settings-modal";

import { Card, CardContent } from "@/components/ui/card";
import { ScrollArea } from "@/components/ui/scroll-area";
import ChatPreview from "../AgentBuilder/ChatPreview";
...
export default function AgentBuilder() {
  ...
  const [chatFormUrl, setChatFormUrl] = useState("");
  const [editFormInput, setEditFormInput] = useState("");
  const [agentGroup, setAgentGroup] = useState("");
  const [selectedStoryName, setSelectedStoryName] = useState("");
  const [storyApiEndpoint, setStoryEndpoint] = useState("");
  const [profilePicUrl, setProfilePicUrl] = useState("");
  const [sharePublicUrl, setSharePublicUrl] = useState(false);
  ...
  const getData = async () => {
    try {
      ...
      const formGroup = result[0]?.form_group;
      setShareUrl(
        `${process.env.NEXT_PUBLIC_APP_URL}/agentmaker/${formUuid}/${formGroup}`
      );
      setChatFormUrl(
        `${process.env.NEXT_PUBLIC_APP_URL}/agentmaker/${formUuid}/${formGroup}`
      );
      setSharePublicUrl(result[0]?.share_url_status);
      if (response) {
  ...
  useEffect(() => {
    if (currentPath && editModeData) {
      ...
      setAgentGroup(editModeData?.form_group || "");
      const usersJson = editModeData?.users_json
        ? Array.isArray(editModeData.users_json)
          ? editModeData.users_json
          : JSON.parse(editModeData.users_json)
        : [];
      setSelectedUsers(usersJson);
      ...
      setSelectedStoryName(editModeData?.story_name || "");
      setStoryEndpoint(editModeData?.story_api || "");
    }
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        content:
          typeof contentData === "string" && contentData.length > 0
            ? JSON.parse(contentData)
            : contentData,
        form_success_url: redirectUrl,
        form_success_message: successMsg,
        share_url: currentPath ? shareUrl : "",
        chatform_url: currentPath ? chatFormUrl : "",
        share_url_status: sharePublicUrl,
        users_json: Array.isArray(selectedUsers) ? selectedUsers : [],
        custom_one: nameFields,
        api_connection_json: apiConnectionJson && JSON.parse(apiConnectionJson),
        db_connection_json: dbConnectionJson && dbConnectionJson,
        doc_connection_json:
          documentConnectionJson && JSON.parse(documentConnectionJson),
        story_name: selectedStoryName,
        story_api: storyApiEndpoint,
      };
      ...
      try {
        ...
        const formFieldsData = {
          ...
          data_api_url: apiEndpoint,
          api_connection_json:
            apiConnectionJson && JSON.parse(apiConnectionJson),
          db_connection_json: dbConnectionJson && dbConnectionJson,
          doc_connection_json:
            documentConnectionJson && JSON.parse(documentConnectionJson),
          form_code: currentPath ? editModeData?.form_code : formCode,
        ...
        try {
          const payload = {
            ...
            user_group_list: agentGroup,
            profile_pic_url: profilePicUrl,
            created_datetime: new Date().toLocaleDateString(),
            form_uuid: formUuid,
            agent_uuid: formUuid,
          };
          const response = await saveToUserCatalog(payload);
          if (!response.success) {
      ...
      <Card>
        <CardContent>
            ...
            <div className="border rounded-lg p-4 mb-8">
              <div className="flex items-center gap-4">
                <Input
                  ...
                  value={currentPath ? editFormInput : formInput}
                  onChange={(e) => {
                    if (currentPath) {
            ...
            <Tabs defaultValue="maker" className="w-full">
              ...
              <TabsContent value="chat_with_data">
                <div className="max-w-[950px] h-full">
                  {formFields?.length > 0 ? (
                    <ChatPreview formFields={formFields} />
                  ) : (
          ...
          <FormSettingsModal
            ...
            shareUrl={shareUrl}
            chatFormUrl={chatFormUrl}
            agentGroup={agentGroup}
            setAgentGroup={setAgentGroup}
            storyApiEndpoint={storyApiEndpoint}
            setStoryEndpoint={setStoryEndpoint}
            selectedStoryName={selectedStoryName}
            setSelectedStoryName={setSelectedStoryName}
            profilePicUrl={profilePicUrl}
            setProfilePicUrl={setProfilePicUrl}
            sharePublicUrl={sharePublicUrl}
            setSharePublicUrl={setSharePublicUrl}
