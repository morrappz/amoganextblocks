...
export function UpdateRecordSheet({
  ...
  React.useEffect(() => {
    form.reset({
      ...
      data_group_category: record?.data_group_category || "",
      data_group_name: record?.data_group_name || "",
      name: record?.name || "",
      short_description: record?.short_description || "",
      remarks: record?.remarks || "",
      data_model_json: record?.data_model_json
        ? JSON.stringify(record.data_model_json, null, 2)
        : "",
      data_combination_json: record?.data_combination_json || "",
      ...
      <DialogContent className="sm:max-w-ld max-h-[90vh] overflow-y-auto">
        ...
        <Form {...form}>
          ...
          >
            ...
            <FormField
              control={form.control}
              name="data_combination_json"
              render={({ field, fieldState }) => (
                <FormItem>
                  <FormLabel className="flex items-center justify-between">
                    <span>Data Combination JSON </span>
                    {fieldState.error && <FormMessage />}
                  </FormLabel>
                  <FormControl>
                    <Textarea
