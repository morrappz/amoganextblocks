...
export function CardsView({ records, table }: RecordsCardProps) {
  ...
  {records.map((record) => {
      const actionCell = getActionCell(record.data_group_id?.toString() ?? "");
    return (
      <Card
        key={record?.data_group_id}
        className="w-full rounded-md relative"
      >
        <CardHeader className="flex items-start space-x-4">
          <div>
            <CardTitle>
              {record?.data_group_name}
            </CardTitle>
        ...
        <CardContent>
          <p>ID: {record?.data_group_id}</p>
          <p>
            Created:{" "}
            {record?.created_date &&
              format(record?.created_date, "PPP")}
          </p>
          <p>Data Group Type: {record?.data_group_type}</p>
        </CardContent>
