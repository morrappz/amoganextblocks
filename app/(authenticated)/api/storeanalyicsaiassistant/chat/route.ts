...
export async function POST(req: Request) {
  ...
  try {
    ...
    const aiProviderSettings = businessSettings.ai_provider_key;
    if (!aiProviderSettings?.provider || !aiProviderSettings?.apiKey) {
      throw new Error("AI provider settings are incomplete.");
    }

    // Ensure site_url does not end with a slash and add WooCommerce API path
    ...
    const config = [
      {
        ...
        type: "woocommerce",
        auth: {
          mparam_ck: woocommerceConfig.credentials.woocommerce.consumer_key,
          mparam_cs: woocommerceConfig.credentials.woocommerce.consumer_secret,
        },
        config: {
          mparam_url: `${siteUrl}/wp-json/wc/v3`,
        },
      },
    ];
    ...
    // Pass AI model based on provider
    let aiModel;
    if (aiProviderSettings.provider === "openai") {
      const openai = createOpenAI({ apiKey: aiProviderSettings.apiKey });
      aiModel = openai("gpt-4o");
    } else if (aiProviderSettings.provider === "gemini") {
    ...
    // Use the selected AI model in the agent
    const agent = new Agent({
      ...
      instructions: `
          ...
          **IMPORTANT** : if you cant get an information based on tools that you have , just answer that you cant dont give me wrong informations. also make sure you provided the flow up Suggestionsto the function
      `,
      model: aiModel,
