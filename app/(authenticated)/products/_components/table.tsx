...
import { ExtendedRowAction, type Product, productStatuses } from "../type";
import type {
...
import { EnhancerRecordSheet } from "./enhancer-sheet";
...
export function ProductsTable({ promises, productGroups }: ProductsTableProps) {
  const { featureFlags } = useFeatureFlags();

  const [{ data, pageCount }, statusCounts] = React.use(promises);

  const [rowAction, setRowAction] = React.useState<ExtendedRowAction>(null);
  ...
  const advancedFilterFields: DataTableAdvancedFilterField<Product>[] = [
    ...
    {
      id: "moving_average_price",
      label: "Average Price",
      type: "number",
    },
    {
      id: "recent_purchase_price",
  ...
  const columnDisplayNames: Record<string, string> = {
    product_small_image_link: "Product Image",
  ...
  const { table } = useDataTable({
    ...
    getRowId: (originalRow) => originalRow.product_id?.toString(),
    shallow: false,
    clearOnDefault: true,
  });
  ...
  return (
    <>
            ...
            endContent={
              <>
                ...
                <Button
                  size="sm"
                  onClick={() =>
                    setRowAction({ row: {} as Row<Product>, type: "new" })
                  }
                >
      ...
      />
      <EnhancerRecordSheet
        open={rowAction?.row?.original && rowAction?.type === "enhancer"}
        onOpenChange={() => setRowAction(null)}
        record={rowAction?.row?.original}
      />
    </>
  );
}
