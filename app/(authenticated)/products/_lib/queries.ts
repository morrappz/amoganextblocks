...
import { productStatuses } from "../type";

const productGroups = ["Product Group", "Product Category"];
...
function applyFilters(
  ...
  if (advancedTable) {
    ...
    if (input.query) {
      query = query.or(
        `product_title.ilike.%${input.query}%,product_category.ilike.%${input.query}%,product_number.ilike.%${input.query}%`
      );
    }
    if (input.from) {
...
export async function getRecords(input: GetProductsSchema) {
  try {
    const { page, perPage, sort, flags } = input;
    const offset = (page - 1) * perPage;
    const advancedTable = flags.includes("advancedTable");

    let query = postgrest.from("product").select("*");
...
export async function getProductGroups() {
  try {
    const { data, error } = await postgrest
      .from("data_group")
      .select("*")
      .in("data_group_type", productGroups)
