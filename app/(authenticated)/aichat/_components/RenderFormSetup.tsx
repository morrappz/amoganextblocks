...
const RenderFormSetup = ({ data }: PageProps) => {
  ...
  return (
    <div className="flex  mb-5 flex-col items-center h-full">
      <div className="border rounded-md sticky top-0 bg-background flex items-center w-full">
      ...
      <div className="grid grid-cols-1 w-full md:grid-cols-2 lg:grid-cols-3  mt-5  gap-5">
        {filteredData.map((item) => (
          <Card key={item.form_id}>
            <CardContent className="flex p-2.5 flex-col">
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <Link href={`/aichat/${item.form_id}`}>
