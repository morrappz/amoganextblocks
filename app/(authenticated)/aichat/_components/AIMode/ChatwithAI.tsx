...
interface PageProps {
  ...
  setIsMessageAction: React.Dispatch<React.SetStateAction<boolean>>;
}
...
const ChatwithData = ({
  ...
  }, [listening, transcript, resetTranscript, setPrompt]);
  ...
  useEffect(() => {
    if (dataProps) {
      ...
      const fetchContextData = async () => {
        if (dataProps?.content?.[0] && dataProps?.content[0]?.type === "api") {
          ...
          for (const item of queries) {
            ...
            setContextData((prev) => [...prev, result]);
          }
        }
        if (
          dataProps?.content &&
          dataProps?.content?.[0] &&
          dataProps?.content[0]?.type === "db"
        ) {
          const queries = dataProps?.content?.[0]?.queries;
          for (const item of queries) {
  ...
  return (
    <div className="md:max-h-[80%] flex h-full flex-col   ">
      <HistoryBar
        open={openHistory}
        data={historyData}
        formId={formId}
