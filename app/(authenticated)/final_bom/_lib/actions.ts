...
export async function createRecord(
  ...
  try {
    ...
    revalidateTag("finalbom");
    revalidateTag("finalbom-status-counts");
...
export async function updateRecord(
  ...
  try {
    ...
    revalidateTag("finalbom");
    if (data?.status === input.status) revalidateTag("finalbom-status-counts");
...
export async function quickUpdateRecord(
  input: QuickUpdateFinalBomSchema & { id: number }
) {
  unstable_noStore();
  try {
    const { data, error } = await postgrest
